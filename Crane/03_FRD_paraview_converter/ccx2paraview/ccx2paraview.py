...
class FRD:
    ...
    def calculate_principal(self, b):
        ...
        for node_num in b.node_block.nodes.keys():
            ...
            # Forzar simetría del tensor para evitar valores complejos
            tensor = 0.5 * (tensor + tensor.T)

            # Calcular eigenvalores
            eigvals = np.linalg.eigvals(tensor)

            # Tomar solo la parte real si hay componentes imaginarias
            eigvals_real = [ev.real for ev in eigvals]

            # Ordenar y guardar
            for ps in sorted(eigvals_real):
