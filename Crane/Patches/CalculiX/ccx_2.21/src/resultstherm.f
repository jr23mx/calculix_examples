...
!
     ...
     &  iponoel(*),inoel(2,*),network,ipobody(2,*),ibody(3,*),ii
!
     ...
     &  xbody(7,*)
!
      real*8 fa,coords_(3,20),cogauss(3),act
      include "gauss.f"
!
      iflag=3
      null=0
...
!
!=========ELEMENT DEACTIVATION/ACTIVIATION
            if(amat .eq.'DEPOSIT') then
      !      write(*,*) istep, iinc
               fa=xstateini(1,kk,i)
               act=xstateini(2,kk,i)
             !   if(time.lt. 0.025) fa=1.d-6
               if((istep.eq. 1).and.(iinc.eq. 1)) then 
                 fa=1.d-6
              ! end if
               elseif(fa.lt. 1.0)then
                 do j=1,3
                  do i1=1,nope
                     coords_(j,i1)=co(j,konl(i1))
                     cogauss(j)=cogauss(j)+shp(4,i1)*co(j,konl(i1))
                  enddo
                 enddo
                call weld_deposit(ttime+time,coords_,cogauss,co,nope,fa)
               end if
               if(fa .gt. .5) fa = 1.d0
               if(fa .lt. .5) then
                do ii=1,6
                 coconloc(ii)=coconloc(ii)*1.d-6
                end do
              !  rho= 0.d0
               end if
               if(fa .gt. 0.5d0) act = 1.d0
               xstate(1,kk,i)=fa
               xstate(2,kk,i)=act
!-----------Powder
            elseif(amat .eq.'POWDER') then
              fa=xstateini(1,kk,i)
              fa=1.d-6
              xstate(1,kk,i)=fa
!..........................
            elseif(amat .eq.'X5CRNI') then
              fa=xstateini(1,kk,i)
              fa=1.d0
              xstate(1,kk,i)=fa
            end if
